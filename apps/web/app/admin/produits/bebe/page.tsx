...
export default function AdminProductsBebePage() {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    if (finalProductData) {
      ...
      alert("Produit Bébé et ses images ajoutés avec succès !");
    }
  ...
  const handleUpdateProduct = async (id: string, newPrice: number, newStock: number, isActive: boolean) => {
    ...
    const { error } = await supabase
      .from("products")
      .update({ price: newPrice, stock: newStock, is_active: isActive, updated_at: new Date().toISOString() })
      .eq("id", id);

    if (error) {
      alert("Erreur de mise à jour : " + error.message);
    } else {
      setProducts(products.map((p) => (p.id === id ? { ...p, price: newPrice, stock: newStock, is_active: isActive } : p)));
    }
    setUpdatingId(null);
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-[#333333]/10">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="p-2 rounded-xl bg-[#6E857B]/20 text-[#333333]">
                <Baby className="w-5 h-5 text-[#6E857B]" />
              </span>
              <h1 className="text-xl font-bold text-[#333333]">
                Gestion Univers Bébé (Soins & Éveil)
              </h1>
            </div>
            <p className="text-xs text-[#333333]/60">
              Gérez les articles de la vitrine Bébé ou ajoutez-en de nouveaux manuellement.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-[#6E857B] text-white hover:bg-[#5a6c64] rounded-xl text-xs font-bold transition-colors inline-flex items-center gap-1.5 shadow-sm"
            >
              {showAddForm ? <X className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
              <span>{showAddForm ? "Fermer" : "Ajouter un produit Bébé"}</span>
            </button>
          </div>
        </div>

        {showAddForm && (
          <div className="bg-white rounded-2xl border border-[#6E857B]/30 p-6 shadow-sm mb-6 animate-in fade-in slide-in-from-top-4">
            <h2 className="text-sm font-bold text-[#333333] mb-4 flex items-center gap-2">
              <Plus className="w-4 h-4 text-[#6E857B]" />
              Nouveau Produit Bébé
            </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-gray-300 text-xs font-medium focus:outline-none focus:border-[#6E857B]"
                    placeholder="Ex: Couches lavables bio..."
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="w-1/2 px-3 py-2 rounded-xl border border-gray-300 text-xs font-bold focus:outline-none focus:border-[#6E857B]"
                    />
                    <input
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-gray-300 text-xs focus:outline-none focus:border-[#6E857B]"
        ...
        <div className="bg-white rounded-2xl border border-[#333333]/10 p-6 shadow-sm">
          {loading ? (
            ...
            <div className="text-center py-20 space-y-3">
              ...
              <p className="text-sm font-medium text-gray-500">Aucun produit trouvé pour l'univers Bébé.</p>
              <button onClick={() => setShowAddForm(true)} className="text-xs font-bold text-[#6E857B] hover:underline block mx-auto">
                      ...
                      <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="p-3">
                          <input
                            ...
                            className="px-2.5 py-1.5 rounded-lg border border-gray-300 w-20 text-xs font-extrabold bg-white text-[#333333]"
                          />
                        </td>
                        ...
                        <td className="p-3">
                          <input
                            ...
                            className="px-2.5 py-1 rounded-lg border border-gray-300 w-16 text-xs font-bold bg-white text-[#333333]"
                          />
                        </td>
